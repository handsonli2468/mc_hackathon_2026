...
# Camera resolution / FPS applied to depth_profile, depth.color_profile, and
# rgb_camera.color_profile for every camera. Format: 'WIDTH,HEIGHT,FPS'.
# Override at launch with: camera_resolution:=480,270,30
# D405 supported color + depth modes (verified via rs-enumerate-devices):
#   '1280,720,30'
#   '848,480,30'   (also supports 60 / 90 fps)
#   '640,480,30'   (also supports 60 / 90 fps)
#   '640,360,30'   (also supports 60 / 90 fps)  <- default
#   '480,270,30'   (also supports 60 / 90 fps)
#   '424,240,30'   (also supports 60 / 90 fps)  <- smallest supported
local_parameters = [{'name': 'camera_resolution', 'default': '640,360,30',
                     'description': 'WIDTH,HEIGHT,FPS applied to color + depth streams on all cameras'},
                    {'name': 'camera_name1', 'default': 'back', 'description': 'camera1 unique name'},
                    ...
                    {'name': 'serial_no1', 'default': os.getenv('BLACK_SERIAL_BACK', '_419122270813'), 'description': 'camera1 serial number'},
                    {'name': 'serial_no2', 'default': os.getenv('BLACK_SERIAL_LEFT', '_218622276534'), 'description': 'camera2 serial number'},
                    ...
                    {'name': 'serial_no3', 'default': os.getenv('BLACK_SERIAL_RIGHT', '_218622276687'), 'description': 'camera3 serial number'},
                    ...
                    {'name': 'serial_no4', 'default': os.getenv('BLACK_SERIAL_FRONT', '_313522301188'), 'description': 'camera4 serial number'}, 
]
...
def duplicate_params(general_params, posix):
    ...
    for param in local_params:
        ...
        if param['original_name'] == 'enable_depth':
            param['default'] = 'true'
        elif param['original_name'] in ('depth_module.depth_profile',
                                        'depth_module.color_profile',
                                        'rgb_camera.color_profile'):
            # Pull from the shared `camera_resolution` launch argument so all
            # three profiles stay in sync and can be overridden in one place.
            param['default'] = LaunchConfiguration('camera_resolution')
...
def launch_static_transform_publisher_node(context : LaunchContext):
    ...
    node5 = node_action(
        ...
        executable = "static_transform_publisher",
        arguments=[
            '-0.01476', '0.125', '0.176', '1.571', '0.5236', '-3.142',
            'base_footprint',
